---
title: "Oat-Pea Biomass Estimation, Random Forest"
author: "Peter Hyde"
date: "2025-02-05"
...
this model is for oat inter_crop 
...
training_data <- model_data %>% 
  ...
  filter(inter_crop == "oat") %>% # only use oat plots for this model 
...
prediction_data <- full_data %>% 
  ...
  filter(inter_crop == "oat") # this model is only for oat plots
...
oat_biomass_predictions <- bind_cols(output_data,pred_output) %>% 
  select(observationUnitName,.pred) %>% 
  dplyr::rename(pred_oat_biomass = .pred)


##write.csv(oat_biomass_predictions,"output/monocrop_pred_oat_biomass.csv",row.names=FALSE)
