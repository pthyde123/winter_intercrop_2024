...
#### another tutorial https://www.r-bloggers.com/2021/10/random-forest-in-r-2/
...
# If training the model takes too long try setting up lower value of N from 500
N=50 #length(X_train)
...
predictions <- predict(rf_gridsearch, X_test)
result <- X_test
result['price'] <- y_test
result['prediction']<-  predictions
head(result)
...
#### tried this with biomass data but found another better and used it
...
# Create features and target
X <- data %>% 
  select(inter_crop,oat_height,pea_height,contains("mean"))
y <- data$oat_biomass
...
# Train the model 
regr <- randomForest(x = X_train, y = y_train , maxnodes = 10, ntree = 500,mtry = 100)
