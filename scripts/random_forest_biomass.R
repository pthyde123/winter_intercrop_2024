...
colnames(spectral_5_12_24_data)
...
subplot_data %>% 
  ...
  count() %>% 
  print(n=95)  # the number of subplots per genotype, divide by 3 to get replicates per genotype

str(subplot_data)

#ID oat biomass outliers  
subplot_data %>%
  select(inter_crop, observationUnitName,`Above ground dry biomass - g|day 136|COMP:0000063`) %>% 
  filter(inter_crop == "oat") %>% 
  filter(`Above ground dry biomass - g|day 136|COMP:0000063` < 10)
## there are oat subplots with 0 biomass, removed below 

#ID pea height outlier
subplot_data %>%
  select(inter_crop, observationUnitName,`Above ground dry biomass - g|day 136|COMP:0000063`,"Pea plant height - cm|day 134|COMP:0000062") %>% 
  filter(inter_crop == "oat") %>% 
  filter(`Pea plant height - cm|day 134|COMP:0000062` > 5) 
...
yield <- Cornell_WinterOatPeaIntercrop_2024_Ithaca %>% 
  
  mutate(plot_name = gsub("\\_subplot_.*", "", observationUnitName)) %>% # get plot name without subplot number for joining
  
  filter(observationLevel == "plot") %>% # keep only subplot level data, most of our data is subplot level
 
  left_join(Cornell_WinterOatPeaIntercrop_2024_Ithaca_plot_meta, join_by(plot_name)) %>% # join T3 data with meta, need inter_crop designation, not found on T3 
  
  filter(germplasmName != "Winter Hayden") %>% # remove winter hayden, it was not planted, plots are pea only

  select(`Grain yield - g/m2|CO_350:0000260`,plot_name)
...
ggplot(aes(`Grain yield - g/m2|CO_350:0000260`,pred_biomass))+
  geom_point() +
  geom_smooth(se = FALSE, method = lm)+
  xlab("Grain Yield")+
